...
namespace DCSFIPS
{
    public class FIPPanelA10C : FIPPanel
    {
        ...
        */
        ...
        public FIPPanelA10C(IntPtr devicePtr, FIPHandler fipHandler) : base(devicePtr, fipHandler)
        {
        }
        ...
        */
        ...
        public override void DcsBiosDataReceived(uint address, uint data)
        {
                ...
                */
                if (_activeFIPPageType == A10FIPPageType.HSI)
                {
                    if (address == _hsiHeadingOutput.Address)
                    {
                        ...
                        var raw = 0;
                        if (_hsiHeadingOutput.GetUIntValue(data) != 0)
                        {
                            raw = 360 - (Convert.ToInt32(_hsiHeadingOutput.GetUIntValue(data)) * 360) / 65535;
                            if (raw == 360)
                            {
                                raw = 0;
                            }
                    ...
                    {
                        var raw = 0;
                        if (_hsiCourseOutput.GetUIntValue(data) != 0)
                        {
                            raw = ((Convert.ToInt32(_hsiCourseOutput.GetUIntValue(data)) * 360) / 65535);
                            if (raw == 360)
                            {
                                raw = 0;
                            }
                    ...
                    {
                        var raw = 0;
                        if (_hsiHeadingBugOutput.GetUIntValue(data) != 0)
                        {
                            raw = (Convert.ToInt32(_hsiHeadingBugOutput.GetUIntValue(data)) * 360) / 65535;
                            if (raw == 360)
                            {
                                raw = 0;
                            }
                    ...
                    {
                        var raw = 0;
                        if (_hsiBearing1Output.GetUIntValue(data) != 0)
                        {
                            raw = (Convert.ToInt32(_hsiBearing1Output.GetUIntValue(data)) * 360) / 65535;
                            if (raw == 360)
                            {
                                raw = 0;
                            }
                    ...
                    {
                        var raw = 0;
                        if (_hsiBearing2Output.GetUIntValue(data) != 0)
                        {
                            raw = (Convert.ToInt32(_hsiBearing2Output.GetUIntValue(data)) * 360) / 65535;
                            if (raw == 360)
                            {
                                raw = 0;
                            }
            ...
            {
                Common.LogError(5932, ex, "Exception in FIPPanelA10C Receive DCS-BIOS data.");
            }
        ...
        protected override void PageCallback(IntPtr device, IntPtr page, byte bActivated, IntPtr context)
        {
                    ...
                    case 1:
                        {
                            ...
                            SetImage(1, new Bitmap(Resources.A10C_HSI_background_power_off), A10FIPPageType.HSI.ToString());
                            break;
                        }
                    case 2:
                        {
                            SetActivePage(A10FIPPageType.VVI);
                            SetImage(2, new Bitmap(Resources.A_10C_VVI_background_02_no_data_copy), A10FIPPageType.VVI.ToString());
                            break;
                        }
                    case 3:
                        {
            ...
            {
                var msg = string.Concat(new object[6]
                {
                    "PageCallback device: 0x",
                    DevicePtr.ToString("x"),
                    " Page: ",
                ...
                Common.LogError(10217, ex, "PageCallback, parameters = " + msg);
            }
        }
                        ...
                        case A10FIPPageType.HSI:
                            {
                                if (_a10CHsiDataHolderClass.DataHasChanged)
                                {
                                    ...
                                    SetImage((uint)A10FIPPageType.HSI, bitmap, A10FIPPageType.HSI.ToString());
                        ...
                        case A10FIPPageType.VVI:
                            {
                                if (_a10CVviDataHolderClass.DataHasChanged)
                                {
                                    ...
                                    SetImage((uint)A10FIPPageType.VVI, bitmap, A10FIPPageType.VVI.ToString());
        ...
        public override void Shutdown()
        {
            ...
            {
                Common.LogError(34344343, ex, "FIPPanelA10C.Shutdown");
            }
        }
        ...
        {
            var finalHSIImage = new Bitmap(_finalHSIImageSize.Width, _finalHSIImageSize.Height);
            try
            {
            ...
            {
                Common.LogError(59232, ex, "Exception in FIPPanelA10C DrawSequenceHSI().");
            }
    ...
    public class A10C_HSI_DataHolderClass
    {
         ...
         */
        private volatile int _updateCycle = 0;
        private volatile int _hsiHeading; // 0x4
        private volatile int _hsiCourse; // 0x2
        private volatile int _hsiHeadingBug; // 0x1
                                             /*
                                              * 
                                              */
        private volatile bool _hsiPowerOffFlag;
        private volatile bool _hsiRangePowerOffFlag;
        private volatile int _hsiCourseDeviation;
        private volatile bool _dataHasChanged;
        private volatile int _hsiCourseAbsolute;
        private volatile int _hsiRangeDigitA;
        private volatile int _hsiRangeDigitB;
        private volatile int _hsiRangeDigitC;
        private volatile int _hsiRangeDigitD;
        private volatile bool _hsiIsOffCourse;
        private volatile bool _hsiFlyingTowardsStation;
        private volatile bool _hsiFlyingFromStation;
        private volatile int _hsiBearing1;
        private volatile int _hsiBearing2;
    ...
    public class A10C_VVI_DataHolderClass
    {
         ...
         * If heading changes, no updated until the two others have changed too
         */
        private volatile int _vviValue;
        /*
         * 
         */
        private volatile bool _dataHasChanged;
