...
namespace DCSFIPS
{
    ...
    public partial class MainWindow : ISaitekPanelListener, IDcsBiosDataListener, IGlobalHandler, IProfileHandlerListener, IUserMessageHandler
    {
        ...
        private string _windowName = "DCSFIPS ";
        ...
        private void WindowLoaded(object sender, RoutedEventArgs e)
        {
            ...
            {
                LoadSettings();
                Common.SetErrorLog(Path.GetTempPath() + "\\DCSFIPS_error_log.txt");
                Common.SetDebugLog(Path.GetTempPath() + "\\DCSFIPS_debug_log.txt");
                DBCommon.SetErrorLog(Path.GetTempPath() + "\\DCSFIPS_error_log.txt");
                DBCommon.SetDebugLog(Path.GetTempPath() + "\\DCSFIPS_debug_log.txt");
                //TODO
        ...
        private async void CheckForNewRelease()
        {
            ...
            try
            {
                var client = new GitHubClient(new Octokit.ProductHeaderValue("DCSFIPS"));
                var task = await client.Repository.Release.GetAll("jdahlblom", "DCSFIPS");
        ...
        private void MenuItemErrorLog_OnClick(object sender, RoutedEventArgs e)
        {
            try
            {
                var file = Path.GetTempPath() + "\\DCSFIPS_error_log.txt";
                if (!File.Exists(file))
                {
                    File.Create(file);
                }
        ...
        private void MenuItemDebugLog_OnClick(object sender, RoutedEventArgs e)
        {
            try
            {
                var file = Path.GetTempPath() + "\\DCSFIPS_debug_log.txt";
                if (!File.Exists(file))
                {
                    File.Create(file);
                }
        ...
        private void MenuItemOpenErrorFolder_OnClick(object sender, RoutedEventArgs e)
        {
            try
            {
